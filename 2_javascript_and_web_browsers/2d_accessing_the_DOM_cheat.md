...
* `window.document.querySelector()`: this method lets you access an HTML element using a CSS selector in a string as an argument. This means the argument could be the tag name, the tag and class or id, the `element > element` selector, or another valid CSS selector. Here are some examples:
...
* `window.document.getElementById()`: this method lets you access an HTML element by the value of its `id` attribute. Here is an example:
  * `window.document.getElementById("specialHeader");`

* Notably, all HTML elements that are returned from these `document` methods are objects specifically designed to represent HTML elements in the DOM.

## Sample HTML Used in Examples
---
