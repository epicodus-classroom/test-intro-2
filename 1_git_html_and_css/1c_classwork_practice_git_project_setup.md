...
A **global** configuration sets a configuration _everywhere_ on a machine — not just in one directory or file. When we create a configuration for just one project directory on a machine, it's called a **local** configuration. 
